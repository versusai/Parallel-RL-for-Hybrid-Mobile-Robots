        ...
        try:
            if not config['test']:
                ...
                if fake_step != step:
                    ...
                    file_object = open(ROOT+'/home/adminutec/catkin_ws/src/Parallel-Hydrone-DRL/log_fake'+config['model']+'_'+agent+'.csv', 'a')
                ...
                for agent in range(num_agents):
                    ...
                    file_object = open(ROOT+'/home/adminutec/catkin_ws/src/Parallel-Hydrone-DRL/log_'+config['model']+'_'+agent+'.csv', 'a')
                    file_object.write(str(logs[aux])+","+str(logs[aux + 1])+","+str(logs[aux + 2])+","+str(step)+","+str(global_step)+'\n')
            else:
                ...
                file_object = open(ROOT+'/home/adminutec/catkin_ws/src/Parallel-Hydrone-DRL/log_test'+config['model']+'.csv', 'a')
                file_object.write(str(logs[0])+","+str(logs[1])+","+str(logs[2])+","+str(logs[3])+","+str(logs[4])+","+str(logs[5])+'\n')
...
if __name__ == "__main__":
    ...
    for i in range(config['num_agents'] if not config['test'] else 1):
        if not i:
            os.system('gnome-terminal --tab --working-directory=WORK_DIR -- bash -c "source /home/adminutec/catkin_ws/devel/setup.bash; export ROS_MASTER_URI=http://localhost:{}; export GAZEBO_MASTER_URI=http://localhost:{}; roslaunch hydrone_aerial_underwater_deep_rl hydrone_deep_rl_{}.launch"'.format(11311 + i, 11341 + i, config['env_stage']))
        else:
            os.system('gnome-terminal --tab --working-directory=WORK_DIR -- bash -c "source /home/adminutec/catkin_ws/devel/setup.bash; export ROS_MASTER_URI=http://localhost:{}; export GAZEBO_MASTER_URI=http://localhost:{}; roslaunch hydrone_aerial_underwater_deep_rl hydrone_deep_rl_{}.launch"'.format(11311 + i, 11341 + i, config['env_stage']))
        time.sleep(2)
    time.sleep(5)
