...
class hydroneEnv(gym.Env):
    def __init__(self, observation_mode=0, env_stage=1, max_env_size=None, continuous=False, observation_size=24,
        ...
        self.pub_cmd_vel = rospy.Publisher('/hydrone_aerial_underwater/cmd_vel'+TURTLE, Twist, queue_size=1)
        self.sub_odom = rospy.Subscriber('/hydrone_aerial_underwater/odometry_sensor1/odometry'+TURTLE, Odometry, self.getOdometry)
    ...
    def step(self, action):
        ...
        while data is None:
            try:
                if self.test_real:
                    data = rospy.wait_for_message('/hydrone_aerial_underwater/scan'+TURTLE, LaserScan, timeout=5)
                else:
